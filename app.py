...
if st.session_state.app_mode == "📊 एडवांस एनालाइज़र (All-in-One)":
    st.title("📈 अल्टीमेट शेयर मार्केट एनालाइज़र (Anti-Crash Pro)")
    ...
    if match: st.session_state.shared_ticker = match.group(1)
    ...
    if do_analysis or st.session_state.auto_run:
        ...
        with st.spinner(f"🚀 {selected_company} का डेटा लाया जा रहा है... (एंटी-ब्लॉक सिस्टम चालू)"):
            
            # 🛡️ 1. RETRY SYSTEM FOR CHARTS/PRICE
            ...
            if data.empty:
                st.error("❌ Yahoo Finance के सर्वर ने अभी अस्थाई रूप से डेटा रोक दिया है। कृपया 2-3 मिनट बाद दोबारा प्रयास करें।")
            else:
                data = calculate_indicators(data)
                last_close = data['Close'].iloc[-1]
                
                # 🛡️ 2. ANTI-CRASH SYSTEM FOR FUNDAMENTALS
                info = {}
                ...
                if not info:
                    st.warning("⚠️ Yahoo ने फंडामेंटल डेटा (P/E, एक्सपर्ट रेटिंग) को अभी ब्लॉक किया है, लेकिन आपके **टेक्निकल चार्ट्स और AI स्ट्रेटेजी 100% काम कर रही है!** नीचे एनालिसिस देखें 👇")
                
                high_52 = info.get('fiftyTwoWeekHigh', 'N/A') if info else 'N/A'
                low_52 = info.get('fiftyTwoWeekLow', 'N/A') if info else 'N/A'
                pe_ratio = info.get('trailingPE', 'N/A') if info else 'N/A'
                market_cap = info.get('marketCap', 'N/A') if info else 'N/A'
                analyst_count = info.get('numberOfAnalystOpinions', 'N/A') if info else 'N/A'
                target_price = info.get('targetMeanPrice', 'N/A') if info else 'N/A'
                ...
                else: final_target = last_close * 1.03  

                if s1 < last_close: stop_loss = s1
                elif s2 < last_close: stop_loss = s2
                else: stop_loss = last_close * 0.98    
                ...
                # --- जो छूट गया था: न्यूज़ सेक्शन (वापस आ गया!) ---
...
# =========================================================
    ...
    if st.button(button_text):
        ...
        if len(results) > 0:
            ...
            st.warning("⚠️ आज कोई परफेक्ट शेयर नहीं मिला या सर्वर बहुत धीमा है। थोड़ी देर बाद कोशिश करें।")
